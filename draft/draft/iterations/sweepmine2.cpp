...
pair<bool, pair<int, int>> do_probabilities() {
    ...
    cout << "BOMBS LEFT: " << num_undecided_bombs << endl;
    ...
    //     int row = boundary[index].first, col = boundary[index].second;
    //     long double probability = score[row][col] / sum_score;
    //     cout << "SCORE " << row << " " << col << ": " << probability << endl;
    // }
...
int main() {
    freopen("sweepmine.in", "r", stdin), freopen("sweepmine.out", "w", stdout);
