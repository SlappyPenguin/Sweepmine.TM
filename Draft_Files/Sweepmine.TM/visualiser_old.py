board_size = board_width = board_height = 800
...
class GridSquares(tk.Tk):
    def __init__(self, R, C, moves, charges):
        super().__init__()
    ...
    def draw_upto(self, upto):
        ...
        if upto != 0 and upto != len(self.moves) - 1:
    ...
    for i in range(len(simulated_grid)):
        for j in range(len(simulated_grid[0])):
            if is_shown[i][j]:
                ...
                simulated_grid[i][j] = -2
            else:
                simulated_grid[i][j] = -1
...
if __name__ == "__main__":
    ...
    R, C, N, T, D = map(int, f.readline().strip().split())
    full_grid = []
    for _ in range(R):
    ...
    simulated_grid = [[-1 for _ in range(C)] for _ in range(R)]
    ...
    while True:
        ...
        if p == 0 and full_grid[r-1][c-1] != -1:
            reveal_tile(r-1, c-1)
        elif p == 1:
            is_flagged[r-1][c-1] = 1
        else:
            #current square becomes -3
            moves.append(((p, r, c), [[cell if (i, j) != (r-1, c-1) else -3 for j, cell in enumerate(row)] for i, row in enumerate(simulated_grid)]))
            break
    ...
    final_grid = [[-1 for _ in range(C)] for _ in range(R)]
    for i in range(R):
        for j in range(C):
            if full_grid[i][j] == -1:
                final_grid[i][j] = -3
    ...
    moves.append((moves[-1][0], [row.copy() for row in final_grid]))

    f.close()
    
    app = GridSquares(R, C, moves, charges)
    app.mainloop()