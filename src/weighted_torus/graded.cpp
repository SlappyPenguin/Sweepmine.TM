/*
Do backtracking to assign a probability that every square is a mine, then pick the least dangerous square
*/
...
void process_input(
    ...
    num_rows = new_num_rows, num_cols = new_num_cols, num_bombs = new_num_bombs, is_torus = new_is_torus;
    ...
    for (int i = 1; i <= num_squares; i++) {
        Square square = {i};
        weight[i] = new_weight[square.row][square.col];
    }
}
...
pair<bool, pair<int, int>> next_move(
    ...
    init();
    process_input(new_num_rows, new_num_cols, new_num_bombs, new_grid, new_weight, new_is_torus);

    make_constraints();
    Move move = do_probability();
