...
static void generate_grid(int x, int y) {
    ...
    while(cnt > 0) {
        ...
        if(state[rx][ry] == FLAG || (rx == x && ry == y)) continue;
        cnt--, state[rx][ry] = FLAG;
    }
    ...
    for(int i = 1; i <= r; i++) {
        for(int j = 1; j <= c; j++) {
            if(state[i][j] == FLAG) continue;
            for(pair<int, int> neigh : get_neighbours(i, j)) state[i][j] += charge[neigh.first][neigh.second] * (state[neigh.first][neigh.second] == FLAG ? 1 : 0);
        }
    }
}
    ...
    for(int i = 1; i <= r; i++) {
        for(int j = 1; j <= c; j++) {
            if(is_flag[i][j]) {
                assert(!is_shown[i][j]);
                ret[i][j] = FLAG;
            } else if(!is_shown[i][j]) ret[i][j] = BLANK;
...
int main() {
    ...
    if(!D) {
        ...
        ouf << move.first << " " << move.second.first << " " << move.second.second << endl;
        if(check_win()) exit(0);
    }
    ...
    while(true) {
        move = next_move(r, c, n, vectorise_grid(), charge, (bool)t);
        ouf << move.first << " " << move.second.first << " " << move.second.second << endl;
        if (move.second.first == 404) {
            int j = 0;
            for (int i = 1; i <= 1e10; i++) j += rand(); // time.sleep equivalent lol
            break;
        }
        if(state[move.second.first][move.second.second] == FLAG && move.first == false) {
            ouf << "DIE" << endl;
            cout << "You died." << endl;
            break;
        }
